...
// Hàm wrap text cho SVG (dựa trên chiều rộng thực tế)
function wrapText(text, maxWidth = 228, font = '16px sans-serif') {
  if (!text) return [];
...
// Layout tree với node cao động
function layoutTreeDynamic(node, depth = 0, x = 0, y = 0, spacingX = 320, spacingY = 32) {
  // Wrap text trước để biết số dòng
  const lines = wrapText(node.content || node.topic, NODE_WIDTH - 32, '16px sans-serif');
  const nodeHeight = lines.length * LINE_HEIGHT + 16; // 8px padding trên/dưới
  let nodes = [];
  let links = [];
  let width = 0;
  ...
  if (!node.children || node.children.length === 0) {
    ...
    node.children.forEach((child) => {
      const { nodes: childNodes, links: childLinks, width: childWidth, height: childHeight } = layoutTreeDynamic(
        child,
        depth + 1,
        childX,
        childY,
    ...
    // Tính vị trí node cha ở giữa các node con
    ...
    width = totalWidth;
    // height = nodeHeight + spacingY + maxChildHeight;
    // Để fit nhiều nhánh, lấy tổng chiều cao các nhánh con lớn nhất
    height = nodeHeight + spacingY + (childHeights.length > 0 ? Math.max(...childHeights) : 0);
  }
  return { nodes, links, width, height };
}

const MindMapSimple = ({ data, style }) => {
  if (!data) return <div style={{ color: '#aaa', textAlign: 'center' }}>No mind map data</div>;
  const { nodes, links } = layoutTreeDynamic(data);
  // Tìm min/max để fit SVG
  ...
  // Tạo map id -> node để lấy vị trí
  const nodeMap = Object.fromEntries(nodes.map(n => [n.branch, n]));

  return (
    <div style={{ width: width, minWidth: '100%', background: "#e7eaff66", borderRadius: 12, ...style }}>
            ...
            <g key={node.branch}>
              ...
              >
                {node.lines.map((line, idx) => (
                  <tspan key={idx} x={node.x - minX + NODE_WIDTH / 2 + PADDING} dy={idx === 0 ? 0 : LINE_HEIGHT}>{line}</tspan>
                ))}
              </text>
            </g>
          );
